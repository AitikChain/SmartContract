...
contract AitikChain_Presale is ReentrancyGuard, Ownable {
    ...
    function buyWithBNB()
        ...
        payable
        checkPresaleId(currentSale)
        checkSaleState(currentSale, bnbToTokens(currentSale, msg.value))
        nonReentrant
        returns (bool)
    {
        uint256 usdAmount = (msg.value * getLatestPrice() * USDT_MULTIPLIER) /
            (BNB_MULTIPLIER * BNB_MULTIPLIER);
        require(
            presale[currentSale].amountRaised + usdAmount <=
                presale[currentSale].UsdtHardcap,
            "Amount should be less than leftHardcap"
